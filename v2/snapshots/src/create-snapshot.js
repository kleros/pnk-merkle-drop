...
export async function createSnapshotCreator({
  ...
  const KlerosCore = JSON.parse(await readFile(new URL(chainIdToContract[chainId], import.meta.url)));
  const klerosCore = new Contract(klerosCoreAddress, KlerosCore.abi, provider);
  ...
  async function getAllStakeSetEvents({ fromBlock, toBlock }) {
    return await getEventsWithTimestamp(klerosCore, klerosCore.filters.StakeSet(), {
      fromBlock,
      toBlock,
      concurrency,
    });
